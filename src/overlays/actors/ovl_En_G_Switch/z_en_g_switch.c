...
void EnGSwitch_Init(Actor* thisx, PlayState* play) {
    ...
    // "index"
    PRINTF2(VT_FGCOL(GREEN) "☆☆☆☆☆ インデックス ☆☆☆☆☆ %x\n" VT_RST, this->type);
    // "save"
    PRINTF2(VT_FGCOL(YELLOW) "☆☆☆☆☆ セーブ\t     ☆☆☆☆☆ %x\n" VT_RST, this->switchFlag);
    switch (this->type) {
        case ENGSWITCH_SILVER_TRACKER:
            PRINTF2("\n\n");
            // "parent switch spawn"
            PRINTF2(VT_FGCOL(GREEN) "☆☆☆☆☆ 親スイッチ発生 ☆☆☆☆☆ %x\n" VT_RST, this->actor.params);
            sCollectedCount = 0;
            this->silverCount = this->actor.params >> 6;
            this->silverCount &= 0x3F;
            // "maximum number of checks"
            PRINTF2(VT_FGCOL(MAGENTA) "☆☆☆☆☆ 最大チェック数 ☆☆☆☆☆ %d\n" VT_RST, this->silverCount);
            PRINTF2("\n\n");
            if (Flags_GetSwitch(play, this->switchFlag)) {
                // This is a reference to Hokuto no Ken
                PRINTF2(VT_FGCOL(GREEN) "☆☆☆☆☆ Ｙｏｕ ａｒｅ Ｓｈｏｃｋ！  ☆☆☆☆☆ %d\n" VT_RST, this->switchFlag);
        ...
        case ENGSWITCH_SILVER_RUPEE:
            PRINTF2("\n\n");
            // "child switch spawn"
            PRINTF2(VT_FGCOL(GREEN) "☆☆☆☆☆ 子スイッチ発生 ☆☆☆☆☆ %x\n" VT_RST, this->actor.params);
            this->colorIdx = 5;
            this->numEffects = 20;
        ...
        case ENGSWITCH_ARCHERY_POT:
            ...
            PRINTF2(VT_FGCOL(GREEN) "☆☆☆☆☆ やぶさめぶち抜き壷 ☆☆☆☆☆ \n" VT_RST);
            ...
            if (this->requiredObjectSlot < 0) {
                Actor_Kill(&this->actor);
                // "what?"
                PRINTF2(VT_FGCOL(MAGENTA) " なにみの？ %d\n" VT_RST "\n", this->requiredObjectSlot);
                // "bank is funny"
                PRINTF2(VT_FGCOL(CYAN) " バンクおかしいしぞ！%d\n" VT_RST "\n", this->actor.params);
            }
...
void EnGSwitch_SilverRupeeTracker(EnGSwitch* this, PlayState* play) {
    ...
    if (sCollectedCount >= this->silverCount) {
        ...
        PRINTF2(VT_FGCOL(GREEN) "☆☆☆☆☆ 時はまさに世紀末〜  ☆☆☆☆☆ %d\n" VT_RST, this->switchFlag);
        // "Last!"
        PRINTF2(VT_FGCOL(GREEN) "☆☆☆☆☆ らすとぉ！          ☆☆☆☆☆ \n" VT_RST);
    ...
    if (this->delayTimer == 0) {
        ...
        if ((this->collider.base.acFlags & AC_HIT) || BREG(8)) {
            ...
            if (gallery->actor.update != NULL) {
                ...
                Sfx_PlaySfxCentered(NA_SE_SY_GET_RUPY);
                // "Yeah !"
                PRINTF2(VT_FGCOL(YELLOW) "☆☆☆☆☆ いぇぇーす！ＨＩＴ！！ ☆☆☆☆☆ %d\n" VT_RST, gallery->hitCount);
